...
function retryCountFromEnv() {
  return Number.parseInt(process.env.DEDENT_PASTE_RELEASE_ASSET_RETRIES ?? '1', 10);
}

function retryDelayMsFromEnv() {
  return Number.parseInt(process.env.DEDENT_PASTE_RELEASE_ASSET_RETRY_DELAY_MS ?? '1000', 10);
}
